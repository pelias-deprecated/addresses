...
var elasticsearch = new Backend( esclient, 'pelias', 'geoname' );
...
function importDatasets( datasets ){
  ...
  var intervalLogId = setInterval( function (){
    logger.info( 'Number of addresses imported: %d', addressId );
  }, 1000 * 10 );
  unifiedAddressStream.on( 'end', function (  ){
    clearInterval( intervalLogId );
  });

  var allowedProperties = Object.keys( schema.mappings.geoname.properties )
    .concat( [ 'id', 'type' ] );
